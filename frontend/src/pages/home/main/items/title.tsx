...
export function ItemTitle({ item, isEdited, titleRef }: { item: itemType, isEdited: boolean, titleRef: any }) {
    return (
        <>
            {isEdited
                ? <>
                    <input className="w-100 border-0 outline-0 p-0 text-break" defaultValue={item.text} placeholder="Title" type="text" name="note-title" ref={titleRef} autoFocus />
                    <button className="btn border-0 ms-auto" data-close>
                        <SVG 
